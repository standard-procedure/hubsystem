...
class Views::Components::Show < Views::Base
  ...
  def view_template
    render Views::Layouts::Application.new(title: "HubSystem", user: @user, nav_active: :settings) do
      Column class: %w[grow-1] do
        Components::SystemPanel(title: "Component Gallery", subtitle: "Visual testing for Phlex components") do
              ...
              Row justify: "start", align: "start", gap: 16 do
                ...
                Column class: "grow-1", gap: 4 do
                  StatusMatrix do |matrix|
                    matrix.item state: :online, href: "#"
                    matrix.item state: :critical
                    48.times { matrix.item state: [:online, :warning, :offline].sample }
                    matrix.item state: :warning, href: "#"
                    matrix.item state: :offline
                  end
                  StatusBar do |status|
                    status.item label: "3 Active", state: :online
                    status.item label: "1 Scannin", state: :alert
